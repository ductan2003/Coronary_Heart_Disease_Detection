...
class GrowingNeuralGas:
    ...
    def initialize_gng(self):
        ...
        self.gng.add_vertex(weight=self.dataset[t0, :].astype(np.float64), error=0.0)
        self.gng.add_vertex(weight=self.dataset[t1, :].astype(np.float64), error=0.0)
    ...
    def learning_position(self):
        for _ in range(0, self.l):
            ...
            random_input = random_input.astype(np.float64)
            ...
            neuron_s1['weight'] = neuron_s1['weight'].astype(np.float64) + (self.eb * (random_input - neuron_s1['weight']))
            for neuron in self.gng.vs[self.gng.neighbors(neuron_s1.index)]:
                neuron['weight'] = neuron['weight'].astype(np.float64) + (self.en * (random_input - neuron_s2['weight']))
    ...
    def learn(self, interval=10):
        ...
        for iter, _ in zip(range(0, self.max_iter), tqdm(range(self.max_iter))):
            ...
            if (iter + 1) % interval == 0:
                num_nodes = self.gng.vcount()
                num_edges = self.gng.ecount()
                self.counts_over_time.append((iter, num_nodes, num_edges))

        return self.gng

    def get_counts_over_time(self):
        """Returns the recorded counts of nodes and edges over time."""
