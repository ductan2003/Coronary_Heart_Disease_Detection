...
class Graph(object):
    def __init__(self):
        ...
        # self.node_positions = nx.spring_layout(self.new_graph, seed=42)  # Position nodes in 2D space using spring layout
        ...
        random_node_1 = Node(input_weight=updated_input_array, output_weight=output, insertion_threshold=1, name="1")
        self.graph.append(random_node_1)
        self.new_graph.add_node(random_node_1.name, matrix=updated_input_array)
        ...
        updated_input_array = np.append(input, new_row, axis=0)
        output = np.random.uniform(0, 1, (1, 4))
        random_node_2 = Node(input_weight=updated_input_array, output_weight=output, insertion_threshold=1, name="2")
        self.graph.append(random_node_2)
        self.new_graph.add_node(random_node_2.name, matrix=updated_input_array)
    ...
    def add_node(self, node):
        ...
        if node not in self.graph:
            ...
            self.new_graph.add_node(node.name, matrix=node.input_weight)

    def remove_node(self, node):
        self.graph.remove(node)
        self.new_graph.remove_node(node.name)
    ...
    #TODO
    def get_q_and_f_node(self, step_name):
        ...
        if (q != None):
            ...
            if (f):
                ...
                if (insert_success):
                    ...
                    self.new_graph.remove_edge(q.name, f.name)
                    r.add_neighbor(q)
                    r.add_neighbor(f)
                    self.new_graph.add_edge(r.name, q.name)
                    self.new_graph.add_edge(r.name, f.name)
                else:
                    # print("insert fail")
    ...
    def check_deletion_criteria(self):
        ...
        if (MODEL_CONSTANT.DELETION_THRESHOLD > min and min_item.get_neighbor_size() >= 2 and min_item.age < MODEL_CONSTANT.MINIMAL_AGE and min_item.get_quality_measure_for_learning() < MODEL_CONSTANT.SUFFICIENT_STABILIZATION):
            ...
            for neighbor in min_item.get_neighbors():
                ...
                self.new_graph.remove_edge(neighbor.get_node().name, min_item.itm)
