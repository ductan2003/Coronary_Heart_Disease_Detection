...
public class EcgCalc {
    ...
    /*--------------------------------------------------------------------------*/
    /*    THE EXACT NONLINEAR DERIVATIVES: DAO HAM                            */
    /*--------------------------------------------------------------------------*/
    private void derivspqrst(double t0,double[] x, double[] dxdt){ //dxdt[1]=x', dxdt[2]=y', dxdt[3]=z'

        int i,k;
        double a0,w0,r0,x0,y0,z0; //a=alpha, w= omega, 
        ...
        a0 = 1.0 - Math.sqrt((x[1]-x0)*(x[1]-x0) + (x[2]-y0)*(x[2]-y0))/r0; // alpha = 1= sqrt(x*x -y*y)

        for(i=1; i<=k; i++)
            xi[i] = Math.cos(ti[i]);// ti = theta
        for(i=1; i<=k; i++)
            yi[i] = Math.sin(ti[i]);   


        zbase = 0.005* Math.sin(2.0*PI*paramOb.getFHi()*t0); //z0(t)= A* sin(2 * PI * f2 * t)

        
        t = Math.atan2(x[2],x[1]);// theta = atan2(y,x)
        dxdt[1] = a0*(x[1] - x0) - w0*(x[2] - y0); //x' = alpha*x - omega * y  
        dxdt[2] = a0*(x[2] - y0) + w0*(x[1] - x0); //y' = alpha * y +omega * x
        dxdt[3] = 0.0;  

        for(i=1; i<=k; i++){
          dt = Math.IEEEremainder(t-ti[i], 2.0*PI); // delta_theta[i] = (theta - theta[i]) mod 2*PI
          dt2 = dt*dt;
          dxdt[3] += -ai[i] * dt * Math.exp(-0.5*dt2/(bi[i]*bi[i])); //delta_z = - sum (a[i]*delta_theta[i]*exp(...))
        }
        dxdt[3] += -1.0*(x[3] - zbase); //z' = z' -(z- z0)
    }
    ...
    {
        int i,j;
        double c1,c2,w1,w2,sig1,sig2,rrmean,rrstd,xstd,ratio; // w1 == f1
        double df;//,dw1,dw2;
        ...
        df = sf/(double)n;// internal sampling frequency/n 
        for(i=1; i<=n; i++)
           w[i] = (i-1)*2.0*PI*df;

        for(i=1; i<=n; i++){//-------------------------The power spectrum S(f); w[i]=f 
          //dw1 = w[i]-w1;
          //dw2 = w[i]-w2;
    ...
    private boolean dorun(){
        ...
        q = (int) Math.rint(paramOb.getSf()/paramOb.getSfEcg()); //internal sampling freq/ ECG sampling freq
        ...
        hrfact =  Math.sqrt(paramOb.getHrMean()/60); //60 degree
        ...
        h = 1.0/(double)paramOb.getSf();//sf: internal sampling frequency
        tstep = 1.0/(double)paramOb.getSfEcg();//sfEcg ECG sampling frequency 
